...
var StarRatingComponent = (function (_super) {
    ...
    Is fired when a star is dobule clicked. And updated the rating value.
    This function returns if the disabled or readOnly
    property is set. If provided it emits the onClick event
    handler with the actual rating value.
     ...
     * @return {?}
     */
    StarRatingComponent.prototype.onStarDblClicked = function (rating, event) {
        //fire onClick event
        if (!this.interactionPossible()) {
            return;
        }
        this.rating = rating - this.step;
        var /** @type {?} */ onClickEventObject = {
            rating: this.rating
        };
        this.saveOnClick(onClickEventObject);
